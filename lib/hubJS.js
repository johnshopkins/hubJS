...
var hubJS = (function (ajax) {
     ...
     */
		events: {
			...
			find: function(data) {
				var data = _library.utility.extend({}, data);
				return _library.get("events", data);
			},
			...
			upcoming: function(count) {
				var data = { per_page: _library.utility.isNumeric(count) ? count : 5 };
				return _library.events.find(data);
			},
			 ...
			 * 
			 * @param  {integer} 	id        	ID of event to lookup other events against
			 * @param  {object} 	data     	Data to be sent to the server
			 * @return {Object}    	promise
			 */
			// related: function(id, data) {
			...
			// 	// get the event data
			// 	var event = _library.events.find({id: id});
			// 	var articlePayload;
				
			// 	// find events with the same tags
			// 	var relatedByTag = event.then(function (payload) {
			// 		articlePayload = payload;
			// 		var tagIds = _library.utility.extractEmbeddedItemIds(articlePayload, "tags");
			// 		var tagData = _library.utility.extend({}, data, { tags: tagIds.join(",") });
			// 		return _library.events.find(tagData);

			// 	}).then(function (payload) {

			// 		// if there are events in the payload, return them and continue in down the pipe
			// 		if (payload._embedded.events) {
			// 			return payload;

			// 		// if there are not events, get events related by topic
			// 		} else {
			// 			var topicIds = _library.utility.extractEmbeddedItemIds(articlePayload, "topics");
			// 			var topicData = _library.utility.extend({}, data, { topics: topicIds.join(",") });
			// 			return _library.events.find(topicData);
			// 		}
